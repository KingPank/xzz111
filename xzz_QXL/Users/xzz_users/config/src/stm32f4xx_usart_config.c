...
//#include "BCBGlobalFunc.h"
  ...
  void u1_SendBytesInfoProc(u8* pSendInfo, u16 nSendCount);
	...
	unsigned char sbus_rx_buffer[18]={0};
	
	u8	pUsartRecvBuff[200];
	u8	pUsartSendBuff[200];
	
  u8 G_Dma_U3_RxBuffer[10];
  u8 G_Dma_U3_TxBuffer[10];

	u8 G_Dma_U1_RxBuffer[13];
	u8 G_Dma_U1_TxBuffer[15];
...
//	NVIC_Init(&nvic);
    ...
    DMA_u1_InitStructure_RX.DMA_Memory0BaseAddr = (u32) G_Dma_U1_RxBuffer;//DMA ???0??  
 ...
 DMA_DeInit(DMA2_Stream7);  
    ...
    DMA_u1_InitStructure_TX.DMA_Memory0BaseAddr = (u32)G_Dma_U1_TxBuffer;//DMA ???0??  
...
void DMA2_Stream5_IRQHandler()
{
	int a;
	  ...
	  //	UART1_ReceiveSize =RECEIVE_BUF_SIZE - DMA_GetCurrDataCounter(DMA1_Stream1); 
	    ...
	    a=AgvRecvData(G_Dma_U1_RxBuffer);
...
//	NVIC_Init(&nvic);
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1,ENABLE);//DMA2????   
    ...
    DMA_InitStructure.DMA_Memory0BaseAddr = (u32)G_Dma_U3_RxBuffer;//DMA ???0??  
 ...
 DMA_DeInit(DMA1_Stream3);  
    ...
    DMA_InitStructure_TX.DMA_Memory0BaseAddr = (u32) G_Dma_U3_TxBuffer;//DMA ???0??  
